...
Команда '%s' не имеет аргументов
.

:TEXT_ERROR_OPT_NOARG#0%
Для команды '%s' необходим аргумент
.
...
Неправильная команда или имя файла - "%s".
.
...
Имя файла слишком длинное. - '%s'
.

:TEXT_ERROR_SELFCOPY#0%
Нельзя скопировать '%s' в самого себя
.
...
История командной строки пуста
.
...
ключ /L еще не реализован.
.

:TEXT_ERROR_U_NOTIMPLEMENTED
ключ /U еще не реализован.
.
...
Не удалось открыть исходный код. - '%s'
.
...
Не удалось прочитать файл из '%s'
.

:TEXT_ERROR_WRITE_FILE#0%
Не удалось записать файл в '%s'
.
...
Введите новую дату (mm%sdd%s[cc]yy): \
.
:TEXT_MSG_ENTER_DATE_EUROPE#1%
Введите новую дату (dd%smm%s[cc]yy): \
.
:TEXT_MSG_ENTER_DATE_JAPANESE#1%
Введите новую дату ([cc]yy%smm%sdd): \
.
...
aabbcccc   a   a       c    b
Был нажат Control-Break.\r
Завершить пакетный файл '%s' (Yes/No/All) ? \
.
...
aabbccdd      d      a    b
Перезаписать '%s' (Yes/No/All/Quit) ? \
.
...
YyNnAaQq{BREAK}{ENTER}{ESC}
aabbccdd      d      a    b
Объединить с '%s' (Yes/No/All/Quit) ? \
.
...
Не удалось получить текущую кодовую страницу из системы.
.
:TEXT_ERROR_SET_CODEPAGE#1
Не удалось изменить текущую кодовую страницу.
.
:TEXT_DISPLAY_CODEPAGE#1%
Текущая кодовая страница: %u.
Системная кодовая страница: %u.
.
...
:TEXT_CMDHELP_ALIAS
Отображает, устанавливает, или удаляет псевдонимы.

ALIAS [variable[=][string]]

	variable  Указывает имя псевдонима.
	string    Задает последовательность символов для присвоения псевдониму.

Ввод ALIAS без параметров отобразит текущие псевдонимы.
.
...
:TEXT_CMDHELP_CHCP
Изменяет или отображает текущую кодовую страницу.

CHCP [nnn]

  nnn   Номер кодовой страницы.

Ввод CHCP без параметра отображает номер текущей кодовой страницы.
.
...
Запускает новую копию оболочки FreeDOS.
...
COMMAND [[drive:]path] [device] [/E:nnnnn] [/L:nnnn] [/U:nnn] [/P] [/MSG]
  ...
  /C command      Выполняет указанную команду и returns.
  /K command      Выполняет указанную команду и продолжает работу.
.

:TEXT_CMDHELP_COPY
Копирует один и более файлов в другое местоположение.
...
for source (using wildcards or file1)Ывн format).
.
...
:TEXT_CMDHELP_DATE#1
Отображает или настраивает дату.

DATE [/D] [дата]

Ввод DATE без параметров отображает текущую дату и предлагает
ввести новую. Чтобы оставить текущую дату, нажмите ENTER.
...
Ввод PATH без параметров отображает текущий путь.
.
...
:TEXT_CMDHELP_TIME#1
Отображает или настраивает системное время.

TIME [/T] [время]

Ввод TIME без параметров отобразит текущее время и предложит
ввести новое. Чтобы оставить текущее время, нажмите ENTER.

/T отображает текущее время.
.

:TEXT_CMDHELP_TRUENAME
Отображает полный путь текущего каталога или указанного файла.

TRUENAME [диск:][путь][файл]
.

:TEXT_CMDHELP_TYPE
Отображает содержимое текстовых файлов.

TYPE [диск:][путь]файл
.

:TEXT_CMDHELP_VER
Отображает версию командной строки FreeDOS и другую информацию.

VER [/R] [/W] [/D] [/C]

 /R         Версия ядра и др. информация.
 /W        Гарантия FreeDOS command shell.
 /D         FreeDOS command shell redistribution information.
 /C         Разработчики FreeDOS command shell.
.
...
Отображает внутреннюю память используемую FreeCOM.
...
Путь назначения COPY не должен содержать символ плюс ('+').
.
...
скопировано %luKB/???KB\r\
.
:TEXT_COPY_COPIED#0%
скопировано %luKB/%luKB\r\
.
